...
(function($)
{
  $.mbMenu = {
	...
	{
		if(jQuery.accelerator)
		{
			jQuery.accelerator.onKeyUp= function (o, event)
			{
				//jQuery.accelerator.calledByKey= false;
			}
			jQuery.accelerator.onKeyDown= function (o, event)	// 37 38 39 40 | 27
			{
				jQuery.accelerator.calledByKey= true;
				event= window.event||event;
				var k= event.which||event.keyCode;
				if((k==32 || k==33 || k==34 || k==40) && typeof event.preventDefault != 'undefined')
				{
				...
				if(k==37 || k==38 || k==39 || k==40 || k==13) // 40=Baixo / 38=cima / 39=direita / 37=esquerda
				{
					//alert($(opener));
					//$(opener).attr('border', '5');
					var mn= current_menu;
					var disp= mn.getElementsByTagName('A')[0];
					//alert(mn.innerHTML);
					var tb= disp.parentNode.parentNode.parentNode.parentNode;
					//alert(k)
					if(k==39) // direita
					{
						var ac= $('[accelerator]');
						if(ac.length>1)
						{
							...
							{
								//alert(ac[i].innerHTML+'\n'+__currentOppenedMenu.innerHTML);
								if(ac[i-1] == __currentOppenedMenu)
									break;
							}
							$(ac[i]).trigger('click');
						}
					...
					if(k==40)
					{
						if($(tb).next().attr('class') == 'separator' || $(tb).next().attr('class').indexOf('disabled')>=0)
						{
							while($(tb).next().attr('class') == 'separator' || $(tb).next().attr('class').indexOf('disabled')>=0)
								tb= $(tb).next();
							$(tb).trigger('mouseout');
							//tb= $(tb).next();
						}
						if($(tb).next('table') != null)
						{
							$(tb).trigger('mouseout');
							if($(tb).next('table').html() != null)
							{
								tb= $(tb).next('table');
								$(tb).trigger('mouseover');
							}else
								$(tb).trigger('mouseover');
						}
					}else if(k==38)
						{
							if($(tb).prev().attr('class') == 'separator' || $(tb).prev().attr('class').indexOf('disabled')>=0)
							{
								while($(tb).prev().attr('class') == 'separator' || $(tb).prev().attr('class').indexOf('disabled')>=0)
									tb= $(tb).prev();
								$(tb).trigger('mouseout');
								//tb= $(tb).prev();
							}
							if($(tb).prev('table') != null)
							{
								$(tb).trigger('mouseout');
								if($(tb).prev('table').html() != null)
								{
									tb= $(tb).prev('table');
									$(tb).trigger('mouseover');
								}else
					...
					$(this.menuContainer).trigger('mouseenter');
					...
					{
						menu= menu[0];
						var menuItem= menu.getElementsByTagName('TABLE')
						//for(x in menu) alert(x+': '+menu[x]);
						menuItens= menu;
					}
				}
			};
  ...
  $.fn.extend({
	...
	//alert($(voice).attr("shortCut"));
	var sc= $(voice).attr("shortCut") || '';
        var line="<table id='"+m+"_"+i+"' class='line"+(isTitle?" title":"")+"' cellspacing='0' cellpadding='0' border='0' style='width:100%; display:table' width='100%'><tr>"+imgPlace+"<td class='voice' nowrap></td><td class='mbShortCut'>"+sc+"</td></tr></table>";
        if(isSeparator)
          ...
          if(!(isText || isTitle || isDisabled)){
            ...
            if (op.options.submenuHoverIntent==0){
              $(opener.menuContainer).find("#"+m+"_"+i)
				...
				{
						if(current_menu)
							$(current_menu).trigger('mouseout');
						current_menu= this;
						//alert(current_menu.innerHTML)
						current_subMenu= opener.menuContainer.actualSubmenu;
						clearTimeout($.mbMenu.deleteOnMouseOut);
						$(this).addClass("selected");
	...
	/***********************/
		if(jQuery.accelerator.calledByKey)
		{
			var tb= this.menuContainer.children('table');
			tb= tb[0];
			while($(tb).attr('class') == 'separator' || $(tb).attr('class').indexOf('disabled')>=0)
				tb= $(tb).next();
			
			//$(tb).trigger('mouseover');
			//alert(/{(.)*/.test($(tb).attr('class')));
			//alert($(tb).attr('class'))
			/*if(tb.tagName)
				alert($(tb.getElementsByTagName('A')[0]).html())*/
			var t= $(tb).children('tbody');
			if($(tb).html().indexOf('class="{menu:')== -1 && $(tb).html().indexOf("class='{menu:")== -1)
				if($('.menuContainer').length<2)
				{
					if(t.length==0 || true)
					{
						$(tb).trigger('mouseover');
